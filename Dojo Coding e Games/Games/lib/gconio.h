...
void box(int x, int y, int length, int height, int type) {
    ...
    if ((x < 0) || (y < 0) || (length < 0) || (height < 0) || ((x + length) > get_screen_columns()) || ((y + height) > get_screen_rows()))
        return;
    else {
        switch (type) {
...
#ifdef WIN32
            case 1:
                sprintf(charset, "%c%c%c%c%c%c", 218, 192, 191, 217, 196, 179);
                break;
            case 2:
                sprintf(charset, "%c%c%c%c%c%c", 201, 200, 187, 188, 205, 186);
                break;
            default:
                sprintf(charset, "%c%c%c%c%c%c", 218, 192, 191, 217, '-', '|');
#endif
        }
